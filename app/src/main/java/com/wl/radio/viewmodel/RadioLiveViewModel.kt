...
class RadioLiveViewModel:ViewModel(),LifecycleObserver,RadioLiveModel.DataResultListener<RadioList>{
    ...
    var radiolistFromIds:MutableLiveData<List<Radio>>? = MutableLiveData()
    ...
    override fun setRadioListByIdsSuccess(radiolist: List<Radio>) {
        radiolistFromIds?.value = radiolist

    }
    fun getRadioInfos(radioDataIds:String){
        radioLiveModel.getRadioInfos(radioDataIds)
    }
